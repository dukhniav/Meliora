...
def setup_logging(config: Configuration) -> None:
    ...
    """
    # Log level
    verbosity = config.VERBOSITY_DEBUG
    logging.root.addHandler(bufferHandler)

    logfile = config.LOGFILE_PATH

    if logfile:
        sys_log = logfile.split(":")
        if sys_log[0] == "syslog":
            ...
            # of the systems.
            address = (
                (sys_log[1], int(sys_log[2])) if len(
                    sys_log) > 2 else sys_log[1] if len(sys_log) > 1 else "/dev/log"
            )
...
def purge_settings(logfile, config: Configuration):
    """
    Purse settings
    """
    purge_handler = TimedRotatingFileHandler(
        ...
        when="D",
        interval=1,
        backupCount=config.LOG_LIFESPAN,
        encoding="utf-8",
        delay=False,
    )
